...
const {stableJSONStringify} = require('./string');
    ...
    const deduped = async function (...args) {
        ...
        const makePromise = (state) => {
            ...
            state.delayed = state.running.catch(() => {}).then(async () => {
                ...
                if (state.queue > 0) {
                    debug(`end of calling deduped function "${fn.name}" with key ${k}: queue = ${state.queue} - 1`);
                    state.queue--;
                } else {
                    debug(`end of calling deduped function "${fn.name}" with key ${k}: queue = ${state.queue}, deleted`);
                    delete states[k];
                }
            });
        };
        if (state.running) {
            if (state.queue < queue) {
                debug(`call deduped function "${fn.name}" with key ${k}: already running, queue = ${state.queue} + 1`);
                state.queue++;
                makePromise(state);
            } else {
                debug(`call deduped function "${fn.name}" with key ${k}: already running, queue = ${state.queue}`);
            }
        } else {
            debug(`call deduped function "${fn.name}" with key ${k}: new, queue = ${state.queue}`);
            makePromise(state);
        }
...
function limit(fn, limit, {debug} = {}) {
    ...
    const next = () => {
        executing--;
        debug(
            `finished executing limited function "${fn.name}", ` +
            `${executing} executing <= ${limit} limit`
        );
        if (queue.length > 0) {
            queue.shift()();
        } else {
    ...
    const run = function ({resolve, resolveWait}, args) {
        executing++;
        debug(
            `execute limited function "${fn.name}" with args ${args}, ` +
            `${executing} executing <= ${limit} limit`
        );
    ...
    const limited = function (...args) {
        lastPromise = new Promise(resolve => {
            if (executing < limit) {
                debug(
                    `call limited function "${fn.name}" with args ${args}: execute immediately, ` + 
                    `${executing} executing < ${limit} limit`
                );
                run.call(this, {resolve}, args);
            } else {
                debug(
                    `call limited function "${fn.name}" with args ${args}: enqueue, ` +
                    `${executing} executing >= ${limit} limit`
                );
