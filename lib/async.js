...
function dedup(fn, {key = (...args) => args, within = 0, limit = 1} = {}) {
    const states = {};
    if (!key) key = () => [];
    ...
    const deduped = async function (...args) {
        ...
        if (state.queue > 0 && !state.promise) {
        ...
        if (state.queue < limit) {
            ...
            state.queue++;
            if (state.promise) {
                state.promise = state.promise.then(makePromise, makePromise);
            } else {
                state.promise = makePromise();
            }
        }
        return state.promise;
    };
    
    deduped.states = states;
    deduped.wait = async (...args) => {
        const k = stableStringify(key(...args), {cycles: true});
        const state = states[k] || {};
        return state.promise && state.promise.then(() => {}, () => {});
    };
